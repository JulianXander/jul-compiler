...
| 7 | ~~Positionelles Destructuring ist im Checker nicht umgesetzt~~ — erledigt: `case 'destructuring'` versucht bei einem Feld ohne Namenstreffer zusätzlich die Position (`dereferenceIndexFromObject`), wie es die Laufzeit auch tut. Test `positional-destructuring-from-list` | — | — |
