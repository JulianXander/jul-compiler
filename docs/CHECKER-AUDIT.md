...
| 0 | **`Not(X)` gilt in `hasReliableTypeError` als zuverlässig, sobald `X` es ist**, obwohl `getTypeError` ein `Not` als Quelle wie als Ziel permissiv behandelt: „kein Fehler“ heißt dort nicht „ist Teilmenge“. Rote Tests: `and-with-complement-is-not-assignable-to-unrelated-type`, `complement-head-after-literal-is-reachable`, `and-with-complement-keeps-both-choices` (Bereich „Not“ in checker.test.ts), dazu `predicate-identity-needs-same-literal` | Stillschweigend falsch: `And(Integer Not(0))` ist einem `Text` zuweisbar, weil für `And` als Quelle ein passender Choice genügt und `Not(0)` fast überall „passt“. Dasselbe im catchAll nach `[Greater(0)]`. `And(Not(0) Integer)` wird zu `Not(0)` normalisiert, `Integer` geht verloren. `[0] … [Not(1)]` meldet den zweiten branch fälschlich als unerreichbar | vermutlich klein: `'not'` in `hasReliableTypeError` auf `false`, und im `And`-Fall von `getTypeErrorAtDepth` zählt ein Choice ohne verlässlichen Fehler nicht als passend. Auswirkung auf Verengung und Snapshot noch zu prüfen |
| 4 | `dereferenceIndexFromObject` (checker.ts) hat keinen `'concat'`-Fall — Gegenstück zu der Lücke, die `dereferenceNameFromObjectType`/`dereferenceUnknownKeyFromObject` bei einem Spread aus einem noch generischen Funktionsparameter hatten (jetzt gefixt, Test `spread-of-generic-list-parameter-keeps-element-type`) | Positionszugriff auf einen offenen Spread-`concat` (z.B. `[...a][1]`, `a` generischer Parameter) fällt im `default`-Zweig auf `undefined` statt die Genericity über die vorhandenen `'nestedReference'`/`'parameterReference'`-Fälle pro Quelle offen zu halten — vermutlich kollabiert das analog auf einen zu groben Typ. Kein Repro aus echtem Code, nur durch Analogieschluss gefunden | klein — derselbe Musterfix (pro Quelle rekursiv dereferenzieren, Ergebnisse vereinigen) wie bei den beiden bereits gefixten Funktionen |
