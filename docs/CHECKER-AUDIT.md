...
| 5 | **Funktionsliterale bekommen ihre Parametertypen nur als direktes Argument aus dem Kontext** — `f: (x: Integer) :> Text = (x) => x`, `[cb = (x) => x]` hinter einem Typguard oder als Argument, `List((x: Integer) :> Text)` und der deklarierte Rückgabetyp: überall bleibt `x` `Any`. Plan in [expected-types.md](expected-types.md) | Der Rückgabetyp wird `Any` und passt auf jeden Zieltyp, jede falsche Rückgabe solcher Callbacks geht stumm durch | mittel — braucht den erwarteten Typ je Ausdruck |
