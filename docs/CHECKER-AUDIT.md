...
| 5 | **Spread von `Or([] X)` in ein Dictionary-Literal ergibt `Any`** — eine Union aus Dictionaries verteilt sich inzwischen über ihre Choices (Tests `dictionary-spread-of-union-*`), eine `Empty`-Choice bricht das aber ab | `x = [...s c = 1]` mit `s: Or([] [a: Integer])` hat den Typ `Any`, jeder Folgefehler an `x` geht still durch. Zur Laufzeit trägt `...undefined` keine Felder bei, `Empty` wäre also eine Choice ohne Felder. Dasselbe gilt für einen Spread von `Empty` allein | klein — `empty` im Spread-Fall von case `'dictionary'` als Choice ohne Felder behandeln |
