...
| 5 | **Spread einer Union in ein Dictionary-Literal ergibt `Any`** | `x = [...s c = 1]` mit `s: Or([a: Integer] [b: Text])` hat den Typ `Any`, mit `s: [a: Integer]` dagegen korrekt `[a: Integer c: 1]`. Damit geht jede Zustandsänderung an einer Discriminated Union ungeprüft durch: `[...s status = §attack§]` gegen eine Variante, die zu `§attack§` zusätzlich `attackerId: Integer` verlangt, meldet nichts, obwohl `attackerId` fehlen kann. Echter Fall: `GameUiState` in yugioh (`main.jul`, alle `[...gameUiState status = …]`). Erwartet wäre die Distribution über die Choices, also die Union der gespreadeten Einzelergebnisse | mittel — Spread-Typisierung pro Choice, Normalisierung der Ergebnis-Union |
