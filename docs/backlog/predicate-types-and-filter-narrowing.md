...
`List(Or(Integer Empty))` nicht auf `List(Integer)`, obwohl das Prädikat es beweist. Heute gilt:
Erkennt der Checker im Rumpf eines Boolean-Callbacks exakt ein `?(param)`-Branching, hängt er
`predicate: PredicateFacts` an den Funktionstyp ([syntax-tree.ts](../src/syntax-tree.ts)) -
...
- **TypeScript 5.5 (2024), "inferred type predicates"** - der einzige bekannte, produktiv
  ...
  JULs Branching-Erkennung, nur mit anderem Mustervokabular. Der Referenzpunkt für Punkt 1 unten.
...
### 2. Welchen Parameter die Fakten meinen
...
## Offen aus der Umsetzung

- Den LSP-Bench gibt es nur ohne Messung vor dem Umbau.
- Was der zusätzliche `tryAssignArgs`-Aufruf je Prädikatprüfung zur Laufzeit kostet, ist nicht
  gemessen. Der Bench misst nur parse und check, dafür wäre `bench-runtime` da.
- `Boolean.Without(true)` heißt `And(Boolean Not(true))`, wird aber noch nicht zu `false`
  vereinfacht (jul-examples, type-checking-test.jul).
...
`getTypeError` in runtime.ts behandelt Literale (`typeof`-Vergleich), getaggte Typwerte
(`_julTypeSymbol`) und Funktionswerte als Prädikat (Aufruf über die Parameterbindung, `=== true`)
bereits einheitlich, und `_isOfType` macht daraus einen Boolean. Das ist praktisch schon die
Laufzeit-Hälfte dieses Features, nur bisher nicht von `filter` & Co. genutzt. Pro Aufrufstelle
reicht ein einmaliger Wrap vor der Schleife:

```ts
const actualPredicate = typeof predicate === 'function'
	? predicate
	: (v: T) => _isOfType(v, predicate);
```
